...
class CameraManager(PyQt5.QtCore.QThread):
        ...
        try:
            while self.cap.isOpened():
                ...
                if ret:
                    ...
                    rgbImage = cv2.resize(rgbImage, list(map(lambda a : a - 1, self.capture_dims)))
                    self.frames.append(frame)

                    h, w, ch = rgbImage.shape
                    bytesPerLine = ch * w
                    convertToQtFormat = PyQt5.QtGui.QImage(rgbImage.data, w, h, bytesPerLine, PyQt5.QtGui.QImage.Format_RGB888)
                    p = convertToQtFormat.scaled(self.capture_dims[0], self.capture_dims[1], PyQt5.QtCore.Qt.KeepAspectRatio)
    ...
    def send_history_message(self):
        ...
        right_label = PyQt5.QtWidgets.QLabel(self.history[:-2], objectName = 'RightHistoryMessage')
